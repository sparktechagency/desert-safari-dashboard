...
const Refund = () => {
  ...
  const [createTermsAndConditions] = useCreateRefundMutation();
  const { data: TermsData, refetch } = useGetAllRefundQuery();
  useEffect(() => {
    if (TermsData?.data?.refundPolicy) {
      setValue(TermsData?.data?.refundPolicy);
    }
  }, [TermsData?.data?.refundPolicy, refetch]);
